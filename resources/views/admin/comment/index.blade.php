				...
				<table data-toggle="table" class="table table-condensed mails">
					...
					<tbody>
						...
						<tr>
							...
							<td><a href="{{ route('admin.comment.state', $val->id) }}"  class="btn waves-effect waves-light btn-xs m-l-5 {{ $val->status == 1 ? 'btn-default' : 'btn-warning' }}">{{ $val->status == 1 ? '显示' : '隐藏' }}</a></td>
							<td>

				...
				<table data-toggle="table" class="table table-condensed">
					...
					<tbody>
						...
						<tr>
							...
							<td>{{ $val->sort }}</td>
							<td><a href="{{ route('admin.category.state', $val->id) }}"  class="btn waves-effect waves-light btn-xs m-l-5 {{ $val->status == 1 ? 'btn-default' : 'btn-warning' }}">{{ $val->status == 1 ? '显示' : '隐藏' }}</a></td>
							<td>
